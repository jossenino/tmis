	...
	*/
	class Client extends MY_Controller{
		public function __construct(){
			parent::__construct();
			$this->data['meta_title'] = 'TMIS - Clientes';
		...
		public function index ()
		{
			...
			$this->breadcrumbs->push('Clientes', 'menu/client');
			$this->data['breadcrumbs'] = $this->breadcrumbs->show();
			log_message('info', 'mostrando breadcrumbs');
			//Load model Clientes
			$this->load->model("menu/client_m");
			log_message('info', 'cargado modelo client');
			// Fetch all clients
			$this->data['clients'] = $this->client_m->get_clients();
			
			// Load view
			$this->data['subview'] = 'menu/client/index';
		...
		{
			//Load model Folders
			log_message('info', 'cargando modelo directorio');
			$this->load->model('menu/folders/folders_m');
			//obtenermos el directorio del usuario
			log_message('info', 'obteniendo el dictorio del usuario');
			$result=$this->folders_m->get_by(array('idUsers'=>$this->session->userdata('idUsers')));
			log_message('info',$result[0]->folderLocation);
			$this->load->helper('general_helper');
			$opts = initialize_elfinder($result[0]->folderLocation);
		...
		{
			// Load view
			$this->data['subview'] = 'menu/client/clientFiles';
			//$this->data['NameView'] = 'Usuarios';
			$this->load->view('main_layout', $this->data);
		}
		public function edit ($id = NULL)
		{
			...
			if ($id) {
				$this->data['client'] = $this->client_m->get($id);
				$newClient = FALSE;
				log_message('info', 'Cliente existe');
				count($this->data['clients']) || $this->data['errors'][] = 'turn could not be found';
			}
			else {
				$this->data['clients'] = $this->client_m->get_new();
				$newClient = TRUE;
				log_message('info', 'Cliente nuevo');
			}
			
			// Set up the form
			$rules = $this->client_m->rules;
			...
			if ($this->form_validation->run() == TRUE) {
				if ($newClient == TRUE) {
					...
					$data = $this->client_m->array_from_post(array('clientName', 'typeDocument', 'numberDocument', 'class', 'phone', 'email', 'direction', 'status','country','state','city','idUsers'));
					$this->client_m->save($data, $id);
				}
			}
			//get pais
			...
			// Load the view
			$this->data['subview'] = 'menu/client/edit';
			$this->load->view('main_layout', $this->data);
		}
		...
		public function _unique_client ($str)
		{
			...
			$this->db->where('clientName', $this->input->post('clientName'));
			!$id || $this->db->where('id !=', $id);
			$profile = $this->client_m->get();
